...
The series and their refs: what a series is, where its branches
live, what each ref means and how far it may move.
...
**Two repositories.**
`duckdb/duckdb-r` is canonical: `main`, the parked stable
baselines, the LTS flavor branch; CRAN and the numbered r-universe
packages publish from here.
...
The buffer is deliberately untested — `each.yaml` triggers on
`*-dev`, never on `*-build` — so vendoring can run ahead while CI
catches up
...
*To deepen: absorb `BRANCHES.md` §§ Branch Overview, Source of
Truth, and Release Cycle Mapping.*

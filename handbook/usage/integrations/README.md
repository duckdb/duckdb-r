...
It also holds its connection until a read has reported its end.
...
That includes a stream whose last batch held exactly `chunk_size` rows: every row has arrived, but no read has reported the end.

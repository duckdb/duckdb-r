...
The machinery that turns one upstream commit into one vendor
commit.
...
for a `duckdb/duckdb@<sha>` subject — refusing, rather than
guessing, when the scan comes up empty.
...
A patch that no longer applies is dropped by the next run —
loudly, as a classified failure, not silently — and patches are
sent upstream as pull requests every once in a while.
...
*To deepen: absorb `scripts/VENDORING.md` §§ scripts, fork-point
rule, and monitoring.*

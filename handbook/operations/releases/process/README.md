...
The release process for one DuckDB release line, as a
**finite state machine (FSM)**: clusters, states, gates, multi-line
coordination, and failure and rollback.
It operates on the branch model of
[`branches/model/`](/handbook/branches/model/README.md) and must
preserve the
**[series invariants](/BRANCHES.md#series-invariants)** at every
step; the reverse-dependency runs are
...
(Moved here unchanged from the former root `RELEASE.md`.)
...
The series loop (`.claude/skills/series-loop.md`)
vendors into `-build`,
consumes into `-dev` in bounded chunks (default 100, `scripts/series-advance.sh`),
...
1. The series loop produces the `vendor: … (tag vX.Y.Z) …` commit on `-dev`; wait for

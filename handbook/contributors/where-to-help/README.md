...
Finding work: the label vocabulary,
and how to turn it into a live list of entry points.
The tracker is the list — a saved query is never out of date,
so no curated selection is kept here.

The label that invites outside work is
...
* `upstream 🐟` — the cause is in the DuckDB engine;
  the fix belongs in `duckdb/duckdb` and arrives here by vendoring
  ([`operations/vendoring/`](/handbook/operations/vendoring/README.md)).
* `reprex` — the report is not yet actionable;
  producing the missing reproducible example is help in its own right
  and needs no build.

...
if (scenario == "materialize") {
  ...
  scenario %in% c("stream_discard", "stream_plain", "stream_sorted_limited")
) {
  ...
  rs <- dbSendQueryArrow(
    con,
    if (scenario == "stream_sorted_limited") q_sorted else q
  )
  repeat {
  ...
  rs <- dbSendQuery(con, q, arrow = TRUE)
  tb <- duckdb::duckdb_fetch_arrow(rs)
  rows <- tb$num_rows
  dbClearResult(rs)
  ...
  if (scenario == "adbc_sorted_limited") {
    invisible(execute_adbc(acon, "SET memory_limit = '200MB'"))
  }
  stmt <- adbc_statement_init(acon)
  adbc_statement_set_sql_query(
    stmt,
    if (scenario == "adbc_sorted_limited") q_sorted else q
  )

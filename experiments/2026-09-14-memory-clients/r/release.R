...
local({
  ...
  r0 <- rss()
  ch <- dbFetchArrowChunk(rs, chunk_size = 2e7)
  r1 <- rss()
  rm(ch)
  r2 <- rss()
  invisible(gc())
...
local({
  ...
  on.exit(dbClearResult(rs))
  r0 <- rss()
  ch <- dbFetchArrowChunk(rs, chunk_size = 1e7)
  r1 <- rss() # half the result, 160 MB
  nanoarrow::nanoarrow_pointer_release(ch)
  r2 <- rss()
  step(
    "B release(): idle ",
    mb(r0),
    ", half-size batch held ",
    mb(r1),
    ", after release ",
    mb(r2),
...
local({
  ...
  r3 <- rss()
  step(
    "C second reference: batch held ",
    mb(r1),
    ", after rm() and gc() with the reference alive ",
    mb(r2),
    ", after dropping it and gc() ",
    mb(r3)
  )
})
...
local({
  ...
  r3 <- rss()
  step(
    "D doubles: batch held ",
    mb(r1),
    ", frame converted ",
    mb(r2),
    " (column is ALTREP: ",
    nanoarrow:::is_nanoarrow_altrep(df$a),
    "), batch released with the frame kept ",
    mb(r3)
  )
})
invisible(gc())
...
local({
  ...
  invisible(gc())
  r4 <- rss()
  ok <- identical(df$s[c(1, 1e7)], c("value-0", "value-9999999"))
  r5 <- rss()
  nanoarrow:::nanoarrow_altrep_force_materialize(df$s)
  r6 <- rss()
  invisible(gc())
  r7 <- rss()
  step(

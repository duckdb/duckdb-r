...
`tzone`, `refused` when the check rejects it, which sends duckplyr back
to dplyr, slower and correct, and `relabeled` when it is
...
label already matches the session's;
but which columns those are is
...
refusals it buys back,
and worse than a difference a build flag would at least make visible.
...
result, the label being per session in DuckDB's model
...
pin makes the relational check correct again:
the round-trip label becomes `timezone_out`,
which is the zone the check already compares against,
so `rel_from_df()` could follow the default with no new
...
`TIMESTAMP` already does, no label at all when `timezone_out` is empty,
would settle those.
...
builds the vendored engine from source instead, linking `parquet` and
...
`TZ=Europe/Zurich` alike, because `GetClientProperties()` falls back to a
...
a `TIMESTAMPTZ` column is labeled
...
[`defaults-pin-session.md`](defaults-pin-session.md) got wrong.
That run pinned the session to `timezone_out` and read the result as a
fix; it is one only
because its `timezone_out` was `"UTC"`. Under
`tz_out_convert = "with"` the read side takes a naive wall clock as UTC
whatever `timezone_out` says, that setting relabelling and nothing more,

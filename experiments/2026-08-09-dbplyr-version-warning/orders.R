...
cat(
  "old dbplyr:",
  as.character(packageVersion("dbplyr", lib.loc = old_lib)),
  "| exports sql_glue():",
  "sql_glue" %in%
    names(readRDS(file.path(old_lib, "dbplyr", "Meta", "nsInfo.rds"))$exports),
  "\n"
)
...
run <- function(label, lines, libs = character()) {
  ...
  writeLines(lines, f)
  env <- if (length(libs)) {
    paste0("R_LIBS=", paste(c(libs, .libPaths()), collapse = ":"))
  } else {
    character()
  }
  ...
  out <- out[
    nzchar(out) &
      !grepl("^(i|ℹ) |This persists|storing downloaded|^<environment", out)
  ]
  cat("--", label, "\n")
  cat(paste0("   ", out), sep = "\n")
  cat("\n")

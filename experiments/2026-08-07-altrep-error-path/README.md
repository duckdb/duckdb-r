...
[#2582](https://github.com/duckdb/duckdb-r/pull/2582) does not close
it. `TouchColumn()` packs a registered data frame's columns to plain
memory during bind, on R's thread, so what the scan later dereferences
is no longer an R vector — but the scan's own `rapi_error_with_context()`
call sites are unchanged, and they are what a shared counter covers.
The rule they sit under is

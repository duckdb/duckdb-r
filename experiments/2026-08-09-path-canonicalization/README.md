...
and no source build, because the questions are about path semantics
...
* mode `0311`, search but no read: `normalizePath(mustWork = TRUE)`
  succeeds, DuckDB succeeds.
* mode `0611`, read but no search: `normalizePath(mustWork = TRUE)`
...
The engine's canonicalization is not what costs;
standing up an instance to reach it is.

...
stat <- function(key) {
  ...
  as.numeric(strsplit(x[grep(key, x)], "\\s+")[[1]][2]) / 1024
}
...
make_frame <- function(n = N) {
  set.seed(1)
  data.frame(a = runif(n), b = runif(n))
}
create <- function() dbExecute(con, "CREATE TABLE t (a DOUBLE, b DOUBLE)")
ctas <- function(from) {
  dbExecute(con, paste0("CREATE TABLE t AS SELECT a, b FROM ", from))
}
base <- NA
t0 <- NA
rows <- NA
...
if (scenario == "frame_dbWriteTable") {
  ...
  df <- make_frame()
  start()
  duckdb::duckdb_register(con, "src", df)
  ...
  duckdb::duckdb_register_arrow(con, "src", tb)
  ctas("src")
  rows <- count()
  ...
  duckdb::duckdb_register_arrow(con, "src", rdr)
  ctas("src")
  rows <- count()
  ...
  nanoparquet::write_parquet(df, f)
  ctas(sprintf("read_parquet('%s')", f))
  rows <- count()
} else if (scenario == "chunks_dbAppendTable") {
  create()
  start()
  for (i in seq_len(N / 1e6)) {
    dbAppendTable(con, "t", make_frame(1e6))
  }
  rows <- count()
} else if (scenario == "stream_stdin_csv") {
  create()
  start()
  dbExecute(con, "COPY t FROM '/dev/stdin' (FORMAT CSV, HEADER false)")
  rows <- count()
} else if (
  scenario %in% c("stream_stdin_arrow", "stream_stdin_arrow_1thread")
) {
  if (scenario == "stream_stdin_arrow_1thread") {
    dbExecute(con, "SET threads = 1")
  }
  start()
  rdr <- arrow::as_record_batch_reader(nanoarrow::read_nanoarrow(file(
    "stdin",
    "rb"
  )))
  duckdb::duckdb_register_arrow(con, "src", rdr)
  ...
  DBI::dbAppendTableArrow(
    con,
    "t",
    nanoarrow::read_nanoarrow(file("stdin", "rb"))
  )
  rows <- count()
  ...
  duckdb::duckdb_register_arrow(
    con,
    "src",
    arrow::open_dataset("/data/src.parquet")
  )
  if (endsWith(scenario, "_count")) {
    rows <- dbGetQuery(con, "SELECT count(*) AS n FROM src")$n
  } else {
    ctas("src")
    rows <- count()
  }
  ...
  start()
  ctas("read_parquet('/data/src.parquet')")
  rows <- count()
} else if (scenario == "file_read_csv") {
  start()
  ctas("read_csv('/data/src.csv')")
  rows <- count()
} else if (scenario == "file_duckdb_read_csv") {
  start()
  duckdb::duckdb_read_csv(con, "t", "/data/src.csv")
  rows <- count()
} else if (scenario == "engine_generate") {
  start()
  ctas(sprintf(
    "(SELECT i::DOUBLE AS a, (i * 2)::DOUBLE AS b FROM range(%d) t(i))",
    as.integer(N)
  ))
  rows <- count()
} else {

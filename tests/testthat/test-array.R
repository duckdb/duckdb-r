...
test_that("arrays of DOUBLE with NULL can be read", {
  ...
  a <- c(10, 11, 12, 13)
  b <- matrix(
    as.double(c(1, 2, 3, 4, 5, 6, 7, NA, 9, NA, 11, 12)),
    nrow = 4,
    ncol = 3
  )
...
test_that("arrays work correctly with UNION ALL queries", {
  ...
  expected_matrix_double <- matrix(
    c(1.1, 4.4, 2.2, 5.5, 3.3, 6.6),
    nrow = 2,
    ncol = 3
  )
...
test_that("arrays work correctly in write/read roundtrip after UNION ALL fix", {
  ...
  dbExecute(
    con,
    "CREATE TABLE test_table AS SELECT 1 as id, [4, 5, 6]::INTEGER[3] as matrix_col
                  UNION ALL

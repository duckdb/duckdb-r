...
test_that("rs_list_objects", {
  ...
  expect_equal(
    rs_list_objects(con),
    data.frame(name = c("main"), type = c("schema"), stringsAsFactors = FALSE)
  )
  expect_equal(
    rs_list_objects(con, schema = "main"),
    data.frame(
      name = c("a", "b"),
      type = c("table", "view"),
  ...
  expect_equal(
    rs_list_objects(con, schema = "fuu"),
    data.frame(name = c("x"), type = c("table"), stringsAsFactors = FALSE)
  )
})
...
test_that("rs_list_columns", {
  ...
  cmp <- data.frame(
    name = c("a", "b", "c"),
    type = c("INTEGER", "VARCHAR", "TIMESTAMP"),
    stringsAsFactors = FALSE
  )
...
test_that("mock observer hooray", {
  ...
  mock <- list(
    connectionOpened = function(host, ...) {

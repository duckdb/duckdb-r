...
test_that("a database in a directory that does not exist fails, naming the path", {
  # Normalizing no longer creates anything, so nothing fails there: the engine
  # refuses the open instead, still inside `duckdb()`, and caches nothing.
  path <- file.path(withr::local_tempdir(), "no-such-directory", "db.duckdb")

  expect_error(duckdb(path), "no-such-directory", fixed = TRUE)

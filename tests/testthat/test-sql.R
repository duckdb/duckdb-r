test_that("query() executes SELECT statements correctly", {
...
test_that("query() works with multiple rows and columns", {
  result <- query("SELECT 1 AS x, 'a' AS y UNION ALL SELECT 2, 'b'")
  expect_s3_class(result, "data.frame")
...
test_that("query() can access data frames as tables", {
  # Create a test data frame in the environment
  test_df <- data.frame(id = 1:3, value = c("a", "b", "c"))

  result <- query("SELECT * FROM test_df ORDER BY id")
  expect_s3_class(result, "data.frame")
...
test_that("query() works with custom connection", {
  custom_con <- dbConnect(duckdb())
  result <- query("SELECT 'custom' AS source", conn = custom_con)
  expect_equal(result$source, "custom")
  dbDisconnect(custom_con, shutdown = TRUE)
})

test_that("query() validates connection", {
  local_edition(3)
...
test_that("exec() executes DDL statements correctly", {
  ...
  # Test CREATE TABLE
  rows_affected <- exec("CREATE TABLE test_exec (id INTEGER, name VARCHAR)")
...
test_that("exec() executes INSERT statements correctly", {
  # Clean up any existing test table
  tryCatch(exec("DROP TABLE IF EXISTS test_insert"), error = function(e) NULL)

  exec("CREATE TABLE test_insert (id INTEGER, name VARCHAR)")

  # Test INSERT
  rows_affected <- exec("INSERT INTO test_insert VALUES (1, 'Alice'), (2, 'Bob')")
  expect_equal(rows_affected, 2)

  # Verify the data was inserted
  result <- query("SELECT COUNT(*) AS count FROM test_insert")
  expect_equal(result$count, 2)

  # Clean up
  exec("DROP TABLE test_insert")
})

test_that("exec() executes UPDATE statements correctly", {
  # Clean up any existing test table
  tryCatch(exec("DROP TABLE IF EXISTS test_update"), error = function(e) NULL)

  exec("CREATE TABLE test_update (id INTEGER, name VARCHAR)")
  exec("INSERT INTO test_update VALUES (1, 'Alice'), (2, 'Bob')")

  # Test UPDATE
  rows_affected <- exec("UPDATE test_update SET name = 'Charlie' WHERE id = 1")
  expect_equal(rows_affected, 1)

  # Verify the update
  result <- query("SELECT name FROM test_update WHERE id = 1")
  expect_equal(result$name, "Charlie")

  # Clean up
  exec("DROP TABLE test_update")
})

test_that("exec() executes DELETE statements correctly", {
  # Clean up any existing test table
  tryCatch(exec("DROP TABLE IF EXISTS test_delete"), error = function(e) NULL)

  exec("CREATE TABLE test_delete (id INTEGER, name VARCHAR)")
  exec("INSERT INTO test_delete VALUES (1, 'Alice'), (2, 'Bob'), (3, 'Charlie')")

  # Test DELETE
  rows_affected <- exec("DELETE FROM test_delete WHERE id = 2")
  expect_equal(rows_affected, 1)

  # Verify the deletion
  result <- query("SELECT COUNT(*) AS count FROM test_delete")
  expect_equal(result$count, 2)

  # Clean up
  exec("DROP TABLE test_delete")
})

test_that("exec() works with custom connection", {
  custom_con <- dbConnect(duckdb())

  rows_affected <- exec("CREATE TABLE test_custom (x INTEGER)", conn = custom_con)
  expect_equal(rows_affected, 0)

  # Verify table exists in custom connection
  result <- query("SELECT name FROM sqlite_master WHERE type='table' AND name='test_custom'", conn = custom_con)
  # Note: DuckDB doesn't have sqlite_master, so let's use a different approach
  tryCatch({
    query("SELECT * FROM test_custom LIMIT 0", conn = custom_con)
    table_exists <- TRUE
  }, error = function(e) {
    table_exists <- FALSE
  })
...
test_that("exec() validates connection", {
  local_edition(3)

  invalid_con <- dbConnect(duckdb())
  dbDisconnect(invalid_con, shutdown = TRUE)

  expect_snapshot(error = TRUE, exec("SELECT 1", conn = invalid_con))
})
...
test_that("functions work together in realistic scenarios", {
  ...
  tryCatch(exec("DROP TABLE IF EXISTS integration_test"), error = function(e) NULL)

  # Create table
  exec("CREATE TABLE integration_test (id INTEGER, name VARCHAR, score DOUBLE)")

  # Insert data
  rows_inserted <- exec("INSERT INTO integration_test VALUES (1, 'Alice', 95.5), (2, 'Bob', 87.2), (3, 'Charlie', 92.1)")
  expect_equal(rows_inserted, 3)

  # Query data
  result <- query("SELECT * FROM integration_test ORDER BY score DESC")
  expect_equal(nrow(result), 3)
...
test_that("error handling works correctly", {
  ...
  result <- query("SELECT 'after_error' AS status")

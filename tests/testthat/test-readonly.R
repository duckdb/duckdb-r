test_that("read_only flag and shutdown works as expected", {
  ...
  callr::r(function(dbdir) {
    con <- DBI::dbConnect(duckdb.1.4::duckdb(), dbdir, read_only = FALSE) # FALSE is the default
    print(con)
    res <- DBI::dbWriteTable(con, "iris", iris)
    DBI::dbDisconnect(con)
    duckdb.1.4::duckdb_shutdown(con@driver)
  }, args = list(dbdir))
  ...
  # con is still alive
  callr::r(function(dbdir) {
    con <- DBI::dbConnect(duckdb.1.4::duckdb(), dbdir, read_only = TRUE)

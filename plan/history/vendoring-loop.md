...
[`handbook/operations/ci/per-commit/`](/handbook/operations/ci/per-commit/README.md) (per-commit CI),
...
| **Trigger** per-commit CI | fire-and-forget dispatch, no cap | `each.yaml` → `scripts/each-rcc.sh` → `gh workflow run rcc -f ref=<sha>` (since superseded by the sharded matrix, [`handbook/operations/ci/per-commit/`](/handbook/operations/ci/per-commit/README.md)) |
...
| **Harvest** logs to ground truth | **delayed**, 4×/day | `rcc-logs.yaml` → `scripts/rcc-logs.sh` → orphan branch `rcc` (`runs2.ndjson`, `logs2/<sha>.log`). Since the sharded matrix landed ([`handbook/operations/ci/per-commit/`](/handbook/operations/ci/per-commit/README.md)) the `each-rcc` legs publish their own records as each commit is decided, and this is the backstop. |
...
> commit-selection semantics were already there. See [`handbook/operations/ci/per-commit/`](/handbook/operations/ci/per-commit/README.md) for

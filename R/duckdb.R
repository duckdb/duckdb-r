...
# A field DuckDB did not supply is dropped rather than stored as `NULL`, so that `names()` on the condition says what is available.
...
rapi_error_base <- function(
  ...
  # The extension INSTALL/LOAD guard throws with context "load_extension" and an empty message;
  # the text is centralized in R (extensions_disabled_error()).
  if (identical(context, "load_extension") && !any(nzchar(message))) {
    message <- paste(extensions_disabled_error(), collapse = " ")
  }

  # The exception type is short, bounded, and worth a reader's attention;
  # the rest of what the engine attached rides on the condition instead (`?duckdb_error`),
  # because `extra_info` can hold a resolved stack trace or a list of candidate names that no error message wants inlined.
  ...
  # A classed condition rather than `stop(<string>)`:
  # without rlang the message has no bullets, but the caller still gets `duckdb_error` and the fields.

...
#'
#' \describe{
#'   \item{`error_type`}{DuckDB's exception type as a string, such as `"BINDER"`, `"PARSER"`, `"CONSTRAINT"`, `"CONVERSION"`,
#'     `"IO"` or `"OUT_OF_MEMORY"`.
#'     This is the field to classify on.
#'     The set is the engine's and grows with it, so treat an unrecognized value as "some other error" rather than failing on it.}
#'   \item{`extra_info`}{A named character vector of whatever else the engine attached to the error,
#'     for instance the position within the query.
#'     Which names appear depends on the error, and empty is a normal answer.}
#'   \item{`context`}{The internal operation that failed, such as `"rapi_prepare"` or `"rapi_execute"`.
#'     Useful to tell a failure at prepare time from one at execution time;
#'     the individual names are an implementation detail and may change.}
#'   \item{`raw_message`}{The message as the engine phrased it, without the exception type prefix and without the display formatting.}
#' }
#'

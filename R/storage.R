...
#
# The CRAN Repository Policy says a package should not write in the user's home filespace,
# "nor anywhere else on the file system apart from the R session's temporary directory".
# The package therefore never *creates* anything in the home directory on its own:
# in a non-interactive session (all package checks) extensions and secrets go under tempdir() unless a `~/.duckdb` already exists
# or a location is set explicitly, so reverse dependencies pass checks with no action.
# `~/.duckdb` is only ever created after an explicit, interactive confirmation from the user.
# Nothing is written to the package/user library.
#
# Why the extension and secret locations are set explicitly (rather than via DuckDB's own `home_directory`):
# setting `home_directory` is both incomplete (it does not move the spill directory)
# and too broad (it is also the base for `~` expansion in user SQL, so it would silently redirect paths like `COPY ... TO '~/out.csv'`).
# We therefore leave `home_directory` alone and set `extension_directory`, `secret_directory`, and `temp_directory` --
# all database-global settings, applied via `duckdb()` -- directly.
#
# The package's tests and runnable examples also avoid the bundled C++ engine on CRAN; see the CRAN guard in tests/testthat.R.
...
#'
#' \describe{
#'   \item{Home directory}{The base DuckDB uses to expand a leading `~` and to derive default sub-locations.
#'     DuckDB setting: `home_directory`.
#'     The package does not set this: doing so would also redirect `~` in user SQL (e.g. `COPY ... TO '~/out.csv'`).
#'     The extension and secret locations below are pointed at the resolved home root directly instead.}
#'   \item{Extension binaries}{Downloaded `*.duckdb_extension` files (e.g. `spatial`, `httpfs`, `h3`).
#'     DuckDB setting: `extension_directory`.
#'     A re-usable cache placed at `<home>/extensions`, where `<home>` is resolved as described below.}
#'   \item{Stored secrets}{Persisted credentials under `stored_secrets`.
#'     DuckDB setting: `secret_directory`.
#'     Placed at `<home>/stored_secrets`, the same `<home>`.}
#'   \item{Temporary / spill files}{Out-of-core intermediates for sorts, hash joins, and similar operations.
...
#'     For an in-memory (`:memory:`) database the engine's own default would spill to `.tmp` in the current working directory,
#'     so the package points it at a fresh per-instance sub-directory below [tempdir()] instead.
#'     Every in-memory instance gets its own spill directory:
#'     instances must not share one,
#'     because the engine's spill file names are deterministic and an instance cleans up its directory when it shuts down.
#'     This is a separate setting from the extension/secret home (see below).}
#'   \item{Logs and profiling output}{Written only when a path is explicitly configured (DuckDB settings `log_query_path`,
#'     `http_logging_output`, profiling output).
#'     They default to *off*, so nothing is written without the user asking, and the user chooses where it goes.}
#'   \item{Database file, WAL, and checkpoints}{Chosen by the user through the `dbdir` argument of [duckdb()].
#'     The package does not manage these.}
#' }
#'
...
#' 1. `~/.duckdb`, if that directory already exists -- the location shared with the DuckDB CLI and other clients;
#' 1. In interactive sessions only, the package offers to create `~/.duckdb` once:
#'    answer "yes" to create and use it, "no" to fall through to the temporary directory below, or cancel the prompt to abort with an error.
...
#'
#' \describe{
#'   \item{Storage-location message}{When the package picked the location itself (a per-session [tempdir()],
#'     or an existing `~/.duckdb`),
#'     `duckdb()` emits an informational message describing where extensions and secrets are going and how to change it.
#'     It is throttled by session type: in an **interactive** session at most once every eight hours (a human can act on it);
...
#'     The message is suppressed entirely when you chose the location yourself --
#'     the `home` or `shared_home` argument, the `duckdb.home` option, or the `DUCKDB_R_HOME` environment variable.
#'     Non-interactively it covers both the temporary directory and an existing `~/.duckdb`;
#'     interactively it is issued only when the user opts out of creating `~/.duckdb`.
#'     It is also suppressed once you have made any explicit `home` or `shared_home` choice earlier in the session:
#'     having set the location explicitly once, you have seen how, so later auto-resolved calls stay quiet.
#'   }
#' }
#'
...
#'   In a non-interactive session (which all `R CMD check` runs are) it uses `tempdir()` by default unless a `~/.duckdb` already exists,
...
#'   Run the check in a subprocess to avoid crashing the main R process if the extension is incompatible with the platform.

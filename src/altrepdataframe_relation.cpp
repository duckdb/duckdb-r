...
AltrepDataFrameRelation::AltrepDataFrameRelation(duckdb::shared_ptr<Relation> p, cpp11::sexp df,
                                                 duckdb::conn_eptr_t con,
                                                 duckdb::shared_ptr<AltrepRelationWrapper> altrep)
    : Relation(p->context, RelationType::EXTENSION_RELATION), dataframe(df), connection(std::move(con)),
...
void AltrepDataFrameRelation::BuildTableRelation() {
	if (!table_function_relation) {
		...
		table_function_relation =
		    connection->conn
		        ->TableFunction("r_dataframe_scan", {Value::POINTER((uintptr_t)(SEXP)dataframe)}, other_params, false)
		        ->Alias(alias);
	}
}

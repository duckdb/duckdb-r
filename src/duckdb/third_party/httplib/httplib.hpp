...
#define CPPHTTPLIB_MAX_LINE_LENGTH 32768
  ...
  #define HTTPLIB_DEPRECATED(msg) __attribute__((deprecated(msg)))
#elif defined(_MSC_VER)
  #define HTTPLIB_DEPRECATED(msg) __declspec(deprecated(msg))
#else
  #define HTTPLIB_DEPRECATED(msg)
#endif
  ...
  struct worker {
    ...
    void operator()() {
      for (;;) {
        ...
        {
          duckdb::unique_lock<std::mutex> lock(pool_.mutex_);
...
inline std::string random_string(size_t length) {
  auto randchar = []() -> char {
    ...
    return charset[static_cast<size_t>(unif_rand() * max_index)];
  };
...
HTTPLIB_DEPRECATED("Use set_path_encode instead")

...
MaterializedQueryResult *AltrepRelationWrapper::GetQueryResult() {
	...
	if (!mat_result) {
		...
		client_config.GetConfig(context).user_settings.SetUserSetting("max_expression_depth", Value::UBIGINT(old_depth * 2));
		duckdb_httplib::detail::scope_exit reset_max_expression_depth(
		    [&]() { client_config.SetUserVariable("max_expression_depth", Value::UBIGINT(old_depth)); });

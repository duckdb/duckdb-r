...
[[cpp11::register]] SEXP rapi_reldf_set_alias(data_frame df, duckdb::conn_eptr_t con, std::string alias) {
	...
	return rapi_reldf_to_altrep(make_external_prot<RelationWrapper>("duckdb_relation", prot, rel->rel->Alias(alias)),
	                            con, df);
}
...
[[cpp11::register]] SEXP rapi_reldf_filter(data_frame df, duckdb::conn_eptr_t con, list exprs) {
	...
	return rapi_reldf_to_altrep(make_external_prot<RelationWrapper>("duckdb_relation", prot, std::move(filter)), con,
	                            df);
}
...
[[cpp11::register]] SEXP rapi_reldf_project(data_frame df, duckdb::conn_eptr_t con, list exprs) {
	...
	return rapi_reldf_to_altrep(make_external_prot<RelationWrapper>("duckdb_relation", prot, std::move(projection)),
	                            con, df);
}
...
[[cpp11::register]] SEXP rapi_reldf_aggregate(data_frame df, duckdb::conn_eptr_t con, list groups, list aggregates) {
	...
	return rapi_reldf_to_altrep(make_external_prot<RelationWrapper>("duckdb_relation", prot, std::move(aggregate)), con,
	                            df);
}

[[cpp11::register]] SEXP rapi_reldf_order(data_frame df, duckdb::conn_eptr_t con, list orders,
                                          r_vector<r_bool> ascending) {
	...
	return rapi_reldf_to_altrep(make_external_prot<RelationWrapper>("duckdb_relation", prot, std::move(order)), con,
	                            df);
}
...
[[cpp11::register]] SEXP rapi_reldf_join(data_frame left, data_frame right, duckdb::conn_eptr_t con, list conds,
	...
	if (join == "left") {
		...
		auto df = rapi_reldf_to_altrep(make_external_prot<RelationWrapper>("duckdb_relation", prot, std::move(res)),
		                               con, left);
	...
	return rapi_reldf_to_altrep(make_external_prot<RelationWrapper>("duckdb_relation", prot, std::move(res)), con,
	                            left);
}
...
[[cpp11::register]] SEXP rapi_reldf_union_all(data_frame left, data_frame right, duckdb::conn_eptr_t con) {
	...
	return rapi_reldf_to_altrep(make_external_prot<RelationWrapper>("duckdb_relation", prot, std::move(setop)), con,
	                            left);
}
...
[[cpp11::register]] SEXP rapi_reldf_limit(data_frame df, duckdb::conn_eptr_t con, int64_t n) {
	...
	return rapi_reldf_to_altrep(
	    make_external_prot<RelationWrapper>("duckdb_relation", prot, make_shared_ptr<LimitRelation>(rel->rel, n, 0)),
	    con, df);
}
...
[[cpp11::register]] SEXP rapi_reldf_distinct(data_frame df, duckdb::conn_eptr_t con) {
	...
	return rapi_reldf_to_altrep(
	    make_external_prot<RelationWrapper>("duckdb_relation", prot, make_shared_ptr<DistinctRelation>(rel->rel)), con,
	    df);
}
...
[[cpp11::register]] SEXP rapi_reldf_set_symdiff(data_frame left, data_frame right, duckdb::conn_eptr_t con) {
	...
	return rapi_reldf_to_altrep(make_external_prot<RelationWrapper>("duckdb_relation", prot, std::move(symdiff)), con,
	                            left);
}
...
[[cpp11::register]] SEXP rapi_reldf_from_sql(duckdb::conn_eptr_t con, const std::string sql) {
	...
	return rapi_reldf_to_altrep(make_external_prot<RelationWrapper>("duckdb_relation", prot, std::move(rel)), con,
	                            R_NilValue);
}
...
[[cpp11::register]] SEXP rapi_reldf_explain(data_frame df, duckdb::conn_eptr_t con, std::string type,
                                            std::string format) {
...
[[cpp11::register]] void rapi_reldf_to_parquet(data_frame df, duckdb::conn_eptr_t con, std::string file_name,
                                               list options_sexps) {
	auto rel = cpp11::as_cpp<cpp11::decay_t<duckdb::rel_extptr_t>>(rapi_rel_from_any_df(con, df, true));
	rel->rel->WriteParquet(file_name, ListToVectorOfValue(options_sexps));
}

[[cpp11::register]] void rapi_reldf_to_csv(data_frame df, duckdb::conn_eptr_t con, std::string file_name,
                                           list options_sexps) {
	auto rel = cpp11::as_cpp<cpp11::decay_t<duckdb::rel_extptr_t>>(rapi_rel_from_any_df(con, df, true));
	rel->rel->WriteCSV(file_name, ListToVectorOfValue(options_sexps));
}

[[cpp11::register]] void rapi_reldf_to_table(data_frame df, duckdb::conn_eptr_t con, std::string schema_name,
                                             std::string table_name, bool temporary) {
	auto rel = cpp11::as_cpp<cpp11::decay_t<duckdb::rel_extptr_t>>(rapi_rel_from_any_df(con, df, true));
	rel->rel->Create(schema_name, table_name, temporary);
}

[[cpp11::register]] void rapi_reldf_insert(data_frame df, duckdb::conn_eptr_t con, std::string schema_name,

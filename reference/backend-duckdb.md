...
#> ℹ Using SQL: SELECT * FROM (FROM '/tmp/RtmpVBOpNw/file6db946a466d9.csv') q01
#>   WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/RtmpVBOpNw/file6db946a466d9.csv"
...
#>   read_csv_auto('/tmp/RtmpVBOpNw/file6db946a466d9.csv')) q02 WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/RtmpVBOpNw/file6db946a466d9.csv"
#> 
#> LINE 2: FROM (FROM read_csv_auto('/tmp/RtmpVBOpNw/file6db946a466d9.csv')) q02
#>                    ^
#> ℹ Context: rapi_prepare
#> ℹ Error type: IO
db_csv %>%

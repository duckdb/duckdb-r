...
#> ℹ Using SQL: SELECT * FROM (FROM '/tmp/RtmpmAtaer/file40204b6bd62a.csv') AS q01
#>   WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/RtmpmAtaer/file40204b6bd62a.csv"
...
#>   read_csv_auto('/tmp/RtmpmAtaer/file40204b6bd62a.csv')) AS q02 WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/RtmpmAtaer/file40204b6bd62a.csv"
#> 
#> LINE 2: FROM (FROM read_csv_auto('/tmp/RtmpmAtaer/file40204b6bd62a.csv')) AS...
#>                    ^
#> ℹ Context: rapi_prepare
#> ℹ Error type: IO
db_csv %>%

...
#> ℹ Using SQL: SELECT * FROM (FROM '/tmp/RtmpACEOBy/file42e35be2302e.csv') AS q01
#>   WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/RtmpACEOBy/file42e35be2302e.csv"
...
#>   read_csv_auto('/tmp/RtmpACEOBy/file42e35be2302e.csv')) AS q02 WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/RtmpACEOBy/file42e35be2302e.csv"
#> 
#> LINE 2: FROM (FROM read_csv_auto('/tmp/RtmpACEOBy/file42e35be2302e.csv')) AS...
#>                    ^
#> ℹ Context: rapi_prepare
#> ℹ Error type: IO
db_csv %>%

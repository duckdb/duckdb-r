...
db %>%
  ...
  select(b)
#> # Source:   SQL [?? x 1]
#> # Database: DuckDB 1.4.4 [unknown@Linux 6.17.0-1010-azure:R 4.6.0/:memory:]
#>   b    
#>   <chr>
#> 1 c    
#> 2 d    
...
#> ℹ Using SQL: SELECT * FROM (FROM '/tmp/RtmpZZ6GsU/file4d0b315cb9cd.csv') q01
#>   WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/RtmpZZ6GsU/file4d0b315cb9cd.csv"
...
#>   read_csv_auto('/tmp/RtmpZZ6GsU/file4d0b315cb9cd.csv')) q02 WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/RtmpZZ6GsU/file4d0b315cb9cd.csv"
#> 
#> LINE 2: FROM (FROM read_csv_auto('/tmp/RtmpZZ6GsU/file4d0b315cb9cd.csv')) q02
#>                    ^
#> ℹ Context: rapi_prepare
#> ℹ Error type: IO
db_csv %>%

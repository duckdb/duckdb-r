...
db %>%
  ...
  select(b)
#> # A query:  ?? x 1
#> # Database: DuckDB 1.4.4 [unknown@Linux 6.17.0-1018-azure:R 4.6.0/:memory:]
#>   b    
#>   <chr>
#> 1 c    
#> 2 d    
...
#> ℹ Using SQL: SELECT * FROM (FROM '/tmp/RtmpTbXx9z/file50681154040c.csv') AS q01
#>   WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/RtmpTbXx9z/file50681154040c.csv"
...
#>   read_csv_auto('/tmp/RtmpTbXx9z/file50681154040c.csv')) AS q02 WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/RtmpTbXx9z/file50681154040c.csv"
#> 
#> LINE 2: FROM (FROM read_csv_auto('/tmp/RtmpTbXx9z/file50681154040c.csv')) AS...
#>                    ^
#> ℹ Context: rapi_prepare
#> ℹ Error type: IO
db_csv %>%

...
db %>%
  ...
  select(b)
#> # Source:   SQL [?? x 1]
#> # Database: DuckDB 1.4.4 [unknown@Linux 6.17.0-1010-azure:R 4.5.3/:memory:]
#>   b    
#>   <chr>
#> 1 c    
#> 2 d    
...
#> ℹ Using SQL: SELECT * FROM (FROM '/tmp/RtmpwjcRZM/file4d7b38088efb.csv') q01
#>   WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/RtmpwjcRZM/file4d7b38088efb.csv"
...
#>   read_csv_auto('/tmp/RtmpwjcRZM/file4d7b38088efb.csv')) q02 WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/RtmpwjcRZM/file4d7b38088efb.csv"
#> 
#> LINE 2: FROM (FROM read_csv_auto('/tmp/RtmpwjcRZM/file4d7b38088efb.csv')) q02
#>                    ^
#> ℹ Context: rapi_prepare
#> ℹ Error type: IO
db_csv %>%

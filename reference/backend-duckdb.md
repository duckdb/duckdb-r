...
#> ℹ Using SQL: SELECT * FROM (FROM '/tmp/RtmpElZD7i/file3acc268242be.csv') q01
#>   WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/RtmpElZD7i/file3acc268242be.csv"
...
#>   read_csv_auto('/tmp/RtmpElZD7i/file3acc268242be.csv')) q02 WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/RtmpElZD7i/file3acc268242be.csv"
#> 
#> LINE 2: FROM (FROM read_csv_auto('/tmp/RtmpElZD7i/file3acc268242be.csv')) q02
#>                    ^
#> ℹ Context: rapi_prepare
#> ℹ Error type: IO
db_csv %>%

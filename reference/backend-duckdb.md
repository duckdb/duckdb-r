...
#> ℹ Using SQL: SELECT * FROM (FROM '/tmp/RtmpkVxAPw/file54ad30ddbf3a.csv') q01
#>   WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/RtmpkVxAPw/file54ad30ddbf3a.csv"
...
#>   read_csv_auto('/tmp/RtmpkVxAPw/file54ad30ddbf3a.csv')) q02 WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/RtmpkVxAPw/file54ad30ddbf3a.csv"
#> 
#> LINE 2: FROM (FROM read_csv_auto('/tmp/RtmpkVxAPw/file54ad30ddbf3a.csv')) q02
#>                    ^
#> ℹ Context: rapi_prepare
#> ℹ Error type: IO
db_csv %>%

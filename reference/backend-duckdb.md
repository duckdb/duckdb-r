...
db %>%
  ...
  select(b)
#> # Source:   SQL [?? x 1]
#> # Database: DuckDB 1.5.1-dev401 [unknown@Linux 6.14.0-1017-azure:R 4.5.3/:memory:]
#>   b    
#>   <chr>
#> 1 c    
#> 2 d    
...
#> ℹ Using SQL: SELECT * FROM (FROM '/tmp/Rtmp5ZHZTz/file4b1a4c0554a4.csv') q01
#>   WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/Rtmp5ZHZTz/file4b1a4c0554a4.csv"
...
#>   read_csv_auto('/tmp/Rtmp5ZHZTz/file4b1a4c0554a4.csv')) q02 WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/Rtmp5ZHZTz/file4b1a4c0554a4.csv"
#> 
#> LINE 2: FROM (FROM read_csv_auto('/tmp/Rtmp5ZHZTz/file4b1a4c0554a4.csv')) q02
#>                    ^
#> ℹ Context: rapi_prepare
#> ℹ Error type: IO
db_csv %>%

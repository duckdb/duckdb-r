...
#> ℹ Using SQL: SELECT * FROM (FROM '/tmp/RtmpUM5v8u/file534e1dafbbd0.csv') q01
#>   WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/RtmpUM5v8u/file534e1dafbbd0.csv"
...
#>   read_csv_auto('/tmp/RtmpUM5v8u/file534e1dafbbd0.csv')) q02 WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/RtmpUM5v8u/file534e1dafbbd0.csv"
#> 
#> LINE 2: FROM (FROM read_csv_auto('/tmp/RtmpUM5v8u/file534e1dafbbd0.csv')) q02
#>                    ^
#> ℹ Context: rapi_prepare
#> ℹ Error type: IO
db_csv %>%

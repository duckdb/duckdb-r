...
#> ℹ Using SQL: SELECT * FROM (FROM '/tmp/Rtmp9aQERG/file42d318f5c3f4.csv') AS q01
#>   WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/Rtmp9aQERG/file42d318f5c3f4.csv"
...
#>   read_csv_auto('/tmp/Rtmp9aQERG/file42d318f5c3f4.csv')) AS q02 WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/Rtmp9aQERG/file42d318f5c3f4.csv"
#> 
#> LINE 2: FROM (FROM read_csv_auto('/tmp/Rtmp9aQERG/file42d318f5c3f4.csv')) AS...
#>                    ^
#> ℹ Context: rapi_prepare
#> ℹ Error type: IO
db_csv %>%

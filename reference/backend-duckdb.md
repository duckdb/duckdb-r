...
#> ℹ Using SQL: SELECT * FROM (FROM '/tmp/Rtmp51oo2m/file434149a116b.csv') AS q01
#>   WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/Rtmp51oo2m/file434149a116b.csv"
...
#>   read_csv_auto('/tmp/Rtmp51oo2m/file434149a116b.csv')) AS q02 WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/Rtmp51oo2m/file434149a116b.csv"
#> 
#> LINE 2: FROM (FROM read_csv_auto('/tmp/Rtmp51oo2m/file434149a116b.csv')) AS...
#>                    ^
#> ℹ Context: rapi_prepare
#> ℹ Error type: IO
db_csv %>%

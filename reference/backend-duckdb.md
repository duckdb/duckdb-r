...
db %>%
  ...
  select(b)
#> # Source:   SQL [?? x 1]
#> # Database: DuckDB 1.5.3 [unknown@Linux 6.17.0-1013-azure:R 4.6.0/:memory:]
#>   b    
#>   <chr>
#> 1 c    
#> 2 d    
...
#> ℹ Using SQL: SELECT * FROM (FROM '/tmp/RtmpVaUu8m/file53712c7ee6a7.csv') q01
#>   WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/RtmpVaUu8m/file53712c7ee6a7.csv"
...
#>   read_csv_auto('/tmp/RtmpVaUu8m/file53712c7ee6a7.csv')) q02 WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/RtmpVaUu8m/file53712c7ee6a7.csv"
#> 
#> LINE 2: FROM (FROM read_csv_auto('/tmp/RtmpVaUu8m/file53712c7ee6a7.csv')) q02
#>                    ^
#> ℹ Context: rapi_prepare
#> ℹ Error type: IO
db_csv %>%

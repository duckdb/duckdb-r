...
#> ℹ Using SQL: SELECT * FROM (FROM '/tmp/Rtmp4p5Ti0/file41e35989100e.csv') AS q01
#>   WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/Rtmp4p5Ti0/file41e35989100e.csv"
...
#>   read_csv_auto('/tmp/Rtmp4p5Ti0/file41e35989100e.csv')) AS q02 WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/Rtmp4p5Ti0/file41e35989100e.csv"
#> 
#> LINE 2: FROM (FROM read_csv_auto('/tmp/Rtmp4p5Ti0/file41e35989100e.csv')) AS...
#>                    ^
#> ℹ Context: rapi_prepare
#> ℹ Error type: IO
db_csv %>%

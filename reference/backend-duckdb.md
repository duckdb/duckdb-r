...
#> ℹ Using SQL: SELECT * FROM (FROM '/tmp/Rtmpe7M51x/file5c167212e9fb.csv') AS q01
#>   WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/Rtmpe7M51x/file5c167212e9fb.csv"
...
#>   read_csv_auto('/tmp/Rtmpe7M51x/file5c167212e9fb.csv')) AS q02 WHERE (0 = 1)
#> Caused by error in `dbSendQuery()`:
#> ! IO Error: No files found that match the pattern "/tmp/Rtmpe7M51x/file5c167212e9fb.csv"
#> 
#> LINE 2: FROM (FROM read_csv_auto('/tmp/Rtmpe7M51x/file5c167212e9fb.csv')) AS...
#>                    ^
#> ℹ Context: rapi_prepare
#> ℹ Error type: IO
db_csv %>%

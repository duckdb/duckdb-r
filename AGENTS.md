...
package can be linked against a system-installed libduckdb instead of
compiling the ~1700 vendored .cpp files. This drops a clean
...
The opt-in only applies to `R CMD INSTALL .` — not to `R CMD build`, since
...
`scripts/install-libduckdb.sh` after a vendor update.

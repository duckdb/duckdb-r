...
What is left here is the branch model in detail — components, diagrams,
the legacy `dev`/`dev-base` layout — and the **[series
invariants](#series-invariants)**, which
...
2.  **Flavor** (`DESCRIPTION`, `R/duckdb-package.R`, `NAMESPACE`,
    ...
    (`duckdb`, `duckdb.1.4`, `duckdb.1.4.dev`, …). The rename surface
    and the mechanism are
...
[`operations/vendoring/`](https://r.duckdb.org/handbook/operations/vendoring/README.md)
below), that pair gives way to the loop’s four refs — `<S>-build`,
`<S>-dev`, `<S>-green`, `<S>-build-base`.
...
which runs `scripts/series-port.sh` on every firing; `sync.yaml` handles
the fork’s `main` separately.
...
enforced by a dev-branch health workflow. The numbers file them here and
are not a way to cite one elsewhere: the release FSM in

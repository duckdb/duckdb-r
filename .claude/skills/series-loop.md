...
**On a forward series, the base's test-side fixes are folded in here.**
`<S>-fwd-build` was replayed out of `<S>-build`,
which holds what the code needs to **compile** — the glue, the patch stack —
and nothing of what it needs to **pass its tests**.
Those live on the base `<S>-dev`,
folded into the commit for the same upstream SHA when CI demanded them,
...
and folds that twin's `tests/`, `man/`, `NAMESPACE` and `R/` changes
into the same commit, with the twin's message and a `Carried from` trailer.
The allow-list *is* the design:
`src/` and `patch/` are `-build`'s by definition,
so a compile fix missing from the buffer is mirrored onto the buffer,
never smuggled onto `-dev` here;
and `R/version.R` is regenerated by vendoring,
so it differs between any two runs of the same SHA
and is noise wearing the shape of a fix.
